...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a Gecho instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		gecho := runGeth(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		gecho.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "gecho", "chaindata")
