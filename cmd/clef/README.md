Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for gecho's account management.
This allows DApps not to depend on gecho's account management. When a DApp wants to sign data it can send the data to
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.echochain/keystore")
...
The general flow for signing a transaction using e.g. gecho is as follows:
![image](sign_flow.png)

In this case, `gecho` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Gecho should be started in `gecho --external_signer localhost:8550`.
    - Currently, the Gecho APIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This
  ...
  retains the original input.
    - The Gecho api should switch to use the same type, and relay `to`-account verbatim to the external api.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus gecho/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
...
The signer listens to HTTP requests on `rpcaddr`:`rpcport`, with the same JSONRPC standard as Gecho. The messages are
  ...
  "params": [
    {
      "accounts": [
        {
          "type": "Account",
          "url": "keystore:///home/bazonk/.echochain/keystore/UTC--2017-11-20T14-44-54.089682944Z--123409812340981234098123409812deadbeef42",
          "address": "0x123409812340981234098123409812deadbeef42"
        },
        {
          "type": "Account",
          "url": "keystore:///home/bazonk/.echochain/keystore/UTC--2017-11-23T21-59-03.199240693Z--cafebabedeadbeef34098123409812deadbeef42",
