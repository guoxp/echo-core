...
import (
	...
	"github.com/echochain/echo-core/crypto/sha3"
...
func sigHash(header *types.Header) (hash common.Hash) {
	...
	rlp.Encode(hasher, []interface{}{
		...
		header.GasUsed,
		header.Time,
		header.Extra[:len(header.Extra)-85], // Yes, this will panic if extra is too short
		header.MixDigest,
		header.Nonce,
	})
	hasher.Sum(hash[:0])
...
func (c *Clique) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if checkpoint && header.Voted != (common.Address{}) {
...
func (c *Clique) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	if number%c.config.Epoch != 0 {
		...
		if len(addresses) > 0 {
			header.Voted = addresses[rand.Intn(len(addresses))]
			if c.proposals[header.Voted] {
