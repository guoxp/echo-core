...
#
# The cluster contains a bootnode, a gecho node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gecho and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gecho node with --datadir pointing at <base-dir>/gecho
# and a single, unlocked account with password "gecho"
start_geth_node() {
  local dir="${base_dir}/gecho"
  mkdir -p "${dir}"

  local password="gecho"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating gecho account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gecho account address"
  fi
  ...
  start_node "gecho" "${GETH_IP}" "$(which gecho)" ${args[@]}
}
...
create_account() {
  ...
  gecho --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
